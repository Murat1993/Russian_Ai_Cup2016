...
public final class MyStrategy implements Strategy {
    ...
    public void move(Wizard self, World world, Game game, Move move) {
        ...
        // Определить нападают ли на нас. И если да, то отступить.
        int maxLife = self.getLife();
        if  (self.getLife() < maxLife) {
            goTo(getPreviousWaypoint());
        }
        ...
        if (random == null) {
            ...
            waypointsByLane.put(LaneType.MIDDLE, new Point2D[]{
                    ...
                    random.nextBoolean()
                        ...
                        : new Point2D(200.0, mapSize - 600.0D),
                    new Point2D(800.0D, mapSize - 800.0D),
                    new Point2D(mapSize - 600.0D, 600.0D)
            });

            waypointsByLane.put(LaneType.TOP, new Point2D[]{
                    new Point2D(100D, mapSize - 100.0D),
                    new Point2D(100D, mapSize - 400.0D),
                    new Point2D(200D, mapSize - 800.0D),
                    new Point2D(200D, mapSize * 0.75D),
                    new Point2D(200D, mapSize * 0.5D),
                    new Point2D(200D, mapSize * 0.25D),
